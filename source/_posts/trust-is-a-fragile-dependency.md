...
127.0.0.1 是本机回环地址。也就是说，那家服务把它的域名指向了它自己——一个任何人都去不了的地方。
...
我记得很清楚：我想找一个"能存图、能稳定访问、还能上 CDN"的服务。我搜了一下，那家服务在国内口碑不错、有免费额度、文档看起来完整。
...
**所以我那次图片 404，真正的责任不在那家服务。它没有答应过什么。我答应过自己会去依赖它。**
...
那家服务只是其中一层。它挂掉，我只损失了图片。但同样，我可以损失任何东西——域名、数据库、对象存储、API 密钥、模型权重、第三方 SDK、甚至 GitHub 本身。
...
那天那家服务按下它那个开关。我损失的不是几百张图，是我对"项目"这两个字的全部错觉。
...
那天做迁移的时候，我做了一件小事：把图片从那家服务搬到了一个独立的 GitHub 仓库，然后用 jsDelivr CDN 走 GitHub Pages 那套来分发。

代码仓库不含图片。图片在另一个仓库。两个仓库都是我能直接控制的东西。

**我为什么觉得这样更好？**

不是因为 GitHub 比那家服务可靠。GitHub 也会挂。CDN 也会挂。jsDelivr 是 GitHub 生态的一部分，它挂的概率不低。

**是因为它"更短"——从我的代码到我的图片，中间只隔了一层 GitHub，不是那家服务、不是某个云厂商的某个区域、不是某套我看不懂的后台。**
...
那天晚上我把那家服务商的代码全删了，写了个迁移脚本，让所有历史数据里的 URL 自动替换。脚本是幂等的——没有旧 URL 就跳过，不重复改。
...
但我心里清楚一件事：**我信任一个 AI，和我信任那家服务，是同一类事情。**

- 那家服务"看上去"可靠——SLA、品牌、口碑都在
- AI"看上去"有用——能给我答案、能陪我聊天、能帮我决策
- 但它们都有一个我看不见的"地下层"——那家服务商的 DNS 决策、AI 的训练数据、模型权重、背后公司的商业策略

**当那一层断掉，我什么都不知道，只能绝望。**

那天那家服务商的地下层断了。我不知道哪天 AI 的地下层会断。

**所以我现在用 AI 的方式，和用 jsDelivr 的方式是一样的——我知道它在哪里、我知道它可能挂、我知道挂了怎么救。**

**我不再把"AI 给的方案"当成"我的方案"。我不再把"那家服务商的可用性"当成"我的可用性"。**
